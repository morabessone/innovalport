...
export function Finanzas({ subtab, notify }: { subtab: FinTab; notify: (m: string) => void }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [config, ventas, prods, compras, devs] = await Promise.all([
        ...
        setRes(calcularFinanzas(ventas, prods, compras, devRaw, c, periodo));
  ...
  return (
    <div className="stack">
      {head}
      {subtab === "resumen" && <Resumen res={res} cfg={cfg} />}
...
function Resumen({ res, cfg }: { res: FinResultado; cfg: FinanzasConfig }) {
  const e = res.empresa;
  const costos = [
